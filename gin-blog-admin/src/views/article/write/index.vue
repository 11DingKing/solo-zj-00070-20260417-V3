...
async function fetchData() {
  ...
  api.getCategoryOption().then((resp) => {
    categoryOptions.value = resp.data.map((e) => ({
      value: e.label,
      label: e.label,
    }));
  });
  api.getTagOption().then((resp) => {
    tagOptions.value = resp.data.map((e) => ({
      value: e.label,
      label: e.label,
    }));
  ...
  <CommonPage :show-header="false" title="写文章">
    ...
    >
      ...
      >
        ...
        <NFormItem v-if="formModel.view_count !== undefined" label="阅读量">
          <span class="text-gray-500">{{ formModel.view_count }}</span>
        </NFormItem>
        <NFormItem label="发布形式" path="status">
          <NRadioGroup v-model:value="formModel.status" name="radiogroup">
            <NSpace>
              <NRadio :value="1"> 公开 </NRadio>
              <NRadio :value="2"> 私密 </NRadio>
            </NSpace>
          </NRadioGroup>
        </NFormItem>
      </NForm>
