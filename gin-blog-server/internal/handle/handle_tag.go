...
// @Param is_delete query bool false "是否已删除"
// @Accept json
// @Produce json
...
func (*Tag) GetList(c *gin.Context) {
	...
	data, total, err := model.GetTagListWithDeleted(GetDB(c), query.Page, query.Size, query.Keyword, query.IsDelete)
	if err != nil {
...
// @Summary 软删除标签（批量）
// @Description 根据 ID 数组软删除或恢复标签
// @Tags Tag
// @Param form body SoftDeleteReq true "软删除请求"
// @Accept json
// @Produce json
// @Success 0 {object} Response[int]
// @Security ApiKeyAuth
// @Router /tag/soft-delete [put]
func (*Tag) UpdateSoftDelete(c *gin.Context) {
	...
	rows, err := model.UpdateTagSoftDelete(GetDB(c), req.Ids, req.IsDelete)
	if err != nil {
		ReturnError(c, g.ErrDbOp, err)
		return
	}
...
// @Summary 删除标签（批量，物理删除）
// @Description 根据 ID 数组物理删除标签
// @Tags Tag
